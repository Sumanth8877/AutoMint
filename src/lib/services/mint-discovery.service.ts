...
export async function discoverMintRequirements(
  ...
  console.log('[mint-discovery] Missing critical fields:', initialMissing, '— running Tier 2');

  // ── Tier 2: Jina + Firecrawl in parallel ──────────────────────────────────
  let tier2Source: DiscoverySource = 'merged';
  try {
    const [jinaResult, firecrawlResult] = await Promise.allSettled([
      fetchViaJina(url),
      fetchViaFirecrawl(url),
    ]);

    // Merge both scraper results (Jina wins ties), then merge onto current
    let scraperMerged: Partial<DiscoveredRequirements> = {};
    ...
    // Fill gaps in current with what scrapers found
    current = merge(current, scraperMerged);
  } catch (err) {
    console.warn('[mint-discovery] Tier 2 threw unexpectedly:', err);
  }

  // ── Check if Tier 3 is needed ─────────────────────────────────────────────
  const afterTier2Missing = computeMissing(current);
  if (afterTier2Missing.length === 0) {
    console.log('[mint-discovery] Tier 2 resolved all critical fields');
    return {
      ...current,
      confidence: computeConfidence(current),
      source: tier2Source,
  ...
  console.log('[mint-discovery] Still missing after Tier 2:', afterTier2Missing, '— running Tier 3 (Browserbase)');

  // ── Tier 3: Browserbase + Playwright ─────────────────────────────────────
  try {
    const bbResult = await fetchViaBrowserbase(url);
    current = merge(current, bbResult);
    console.log('[mint-discovery] Tier 3 complete');
  } catch (err) {
    console.warn('[mint-discovery] Tier 3 (Browserbase) failed:', err);
