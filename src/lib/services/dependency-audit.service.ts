...
const MODERNIZATION_PATTERNS: ModernizationOpportunity[] = [
  {
    package: 'next',
    type: 'deprecated-api',
    description: 'Verify all getServerSideProps and getStaticProps usages have been migrated to App Router data-fetching patterns.',
    recommendation: 'Audit pages/ directory for legacy data-fetching functions and migrate to React Server Components + fetch().',
    effort: 'high',
  ...
  },
  {
    package: 'react',
    type: 'deprecated-api',
    description: 'React 19 removes createFactory, isValidElement type overloads, and the legacy Context API pattern.',
  ...
  },
  {
    ...
    type: 'deprecated-api',
    description: 'Clerk v7+ replaced authMiddleware with clerkMiddleware.',
    recommendation: 'Ensure middleware uses clerkMiddleware() from @clerk/nextjs/server.',
    effort: 'medium',
  ...
  },
  {
    package: 'drizzle-orm',
    type: 'deprecated-api',
    description: 'Drizzle ORM 0.30+ deprecated .toSQL() shorthand.',
    recommendation: 'Replace .toSQL() calls with explicit prepared statements.',
    effort: 'low',
  ...
  },
  {
    ...
    description: 'drizzle-kit push is not safe for production — may drop columns.',
    recommendation: 'Switch vercel.json buildCommand from db:push to db:migrate.',
    effort: 'medium',
  ...
  },
  {
    package: 'viem',
    type: 'deprecated-api',
    description: 'Viem 2.x clients should use http() or webSocket() transports.',
    recommendation: 'Audit all createPublicClient / createWalletClient calls.',
    effort: 'low',
  ...
  },
  {
    ...
    type: 'performance',
    description: 'Upstash Redis SDK 1.30+ supports pipeline batching.',
    recommendation: 'Use redis.pipeline() for lock-acquire + expire sequences.',
    effort: 'medium',
  ...
  },
  {
    ...
    type: 'performance',
    description: 'Neon serverless driver supports connection pooling via Pool.',
    recommendation: 'Consider Pool instead of direct neon() connections for high-concurrency routes.',
    effort: 'medium',
